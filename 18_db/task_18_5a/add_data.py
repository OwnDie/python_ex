...
def delete_7_days_inactive_records(db_filename):
    ...
    for row in data:
        try:
            with connection:
                ...
                connection.execute("UPDATE dhcp set active = 1 WHERE mac = ?", (row[0],))
                connection.execute("UPDATE dhcp set last_active = datetime('now') WHERE mac = ?", (row[0],))
...
if __name__ == '__main__':
    if db_exist(db_filename):
        delete_7_days_inactive_records(db_filename)
